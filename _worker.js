...
(function(){
  ...
  function open(anchor){
    const media=anchor.closest('.review-media');
    gallery=[...media.querySelectorAll('a')].map(item=>item.getAttribute('href')).filter(Boolean);
    current=Math.max(0,gallery.indexOf(anchor.getAttribute('href')));
    render();
  ...
  document.addEventListener('click',event=>{
    const anchor=event.target.closest('.review-media a');
    if(anchor){event.preventDefault();open(anchor);return;}
    if(event.target.closest('.review-lightbox-close'))close();
    if(event.target.closest('.review-lightbox-prev'))move(-1);
    if(event.target.closest('.review-lightbox-next'))move(1);
  ...
  normalizeReviewUi();
  setTimeout(normalizeReviewUi,300);
  new MutationObserver(normalizeReviewUi).observe(document.body,{childList:true,subtree:true});
})();
</script>`;
